...
export default function Home() {
  ...
  const [bookmarkedCharts, setBookmarkedCharts] = useState<Set<string>>(new Set());
  const [lastBookmarkedByCategory, setLastBookmarkedByCategory] = useState<Record<string, string>>({});
        ...
        if (data.success) {
          ...
          if (airportList.length > 0) {
            ...
            if (airportCharts) {
              ...
              let airportDiagram = taxiCharts.find((chart: ChartData) => 
                chart.ChartName === '机场图'
              );
  ...
  const handleAirportChange = (airport: string) => {
    ...
    if (airportCharts) {
      ...
      // Try to find exact match of "机场图" first
      let airportDiagram = taxiCharts.find(chart => 
        chart.ChartName === '机场图'
      );
  ...
  const handleCategoryChange = (category: ChartCategory) => {
    ...
    const currentChartIsFromThisCategory = selectedChart && 
      groupedCharts[selectedAirport]?.[category]?.some(c => c.ChartId === selectedChart.ChartId);
    ...
    const bookmarkedChartsInCategory = charts.filter(c => bookmarkedCharts.has(c.ChartId));
    ...
    if (bookmarkedChartsInCategory.length > 0) {
      ...
      if (lastBookmarkedId) {
        const lastBookmarked = bookmarkedChartsInCategory.find(c => c.ChartId === lastBookmarkedId);
        if (lastBookmarked) {
