...
- **新增 Cesium 3D 地球视图**：
  - 主界面新增 3D 地球模式，可在航图列表和 PDF 查看器之外查看机场、程序和地图覆盖层
  - 集成 Cesium 静态资源、Worker 和样式资源，并在 Tauri/Next.js 环境中配置资源加载路径
  - 新增中文和英文的地球视图、覆盖层、初始化提示和状态文案
- **新增高精度程序图地理参考覆盖层**：
  - 新增航图地理参考计算逻辑，可将程序图匹配到地球视图中的地理坐标范围
  - 增加基于航图符号库的匹配流程、符号模板资源和调试渲染工具
  - 支持预加载并缓存符号匹配生成的航图覆盖层，减少重复初始化成本
  - 当缓存地图尚未初始化时，会提示用户执行初始化流程
- **新增 GDL90 Ownship 位置显示**：
  - 新增 GDL90 数据解析和订阅逻辑，可在地球视图中显示本机位置
  - Tauri 后端新增本机位置数据读取接口，并扩展配置类型支持 GDL90 输入

### 变更

- **构建与发布流程支持地理参考 sidecar**：
  - 新增 `scripts/build-georef-sidecar.mjs` 和 Python 依赖清单，用于构建航图地理参考 sidecar
  - GitHub Actions 正式版和预发布版工作流会在测试和打包前构建 georef sidecar 依赖
  - 发布脚本在开始发布前要求工作区保持干净，避免未提交文件混入发布
- **仓库文档和自动化整理**：
  - 刷新 README、贡献指南、安全说明、Issue 模板和 PR 模板
  - 新增发布产物校验脚本，强化 Tauri 安装包发布前检查

### 修复

- **修复地理参考和 PDF 运行时稳定性问题**：
  - 加强地理参考 sidecar、PDF 渲染和运行时资源加载的错误处理
  - 改进图表覆盖层生成流程，并优先匹配带标签的航图符号，提升覆盖层定位可靠性
- **修复图表列表跑道筛选统计问题**：
  - 图表分类数量复用跑道分组逻辑，避免分类计数与当前跑道筛选结果不一致
  - 仅统计当前可见的跑道筛选图表，减少侧边栏数量显示偏差
- **修复长图表名称布局问题**：
  - 图表列表中的长名称会自动换行，避免挤压或溢出列表项
- **修复地球视图交互和资源加载问题**：
  - 规范化地球视图滚轮导航行为，改善缩放和移动体验
  - Tauri 内容安全策略允许加载地图资源
  - 为 Cesium SPZ loader 提供构建兼容 stub，避免构建阶段解析失败
- **修复对话框和控件可访问性问题**：
  - 改善设置、更新通知等对话框与控件的标签、状态和交互语义

### 安全

- **收紧 Tauri updater 权限**：
  - 限制默认 capabilities 中的 updater 权限范围，减少不必要的运行时权限暴露
...
- **PDF 查看器支持顺时针旋转**：
  - 工具栏新增旋转按钮，可按 90 度步进顺时针旋转当前 PDF 页面
  - 旋转渲染会同步更新页面尺寸和画布视口，适配横向或方向异常的航图
  - 切换图表或重新加载文档时会重置旋转角度，避免影响后续图表查看
  - 新增英文和中文旋转按钮提示文案
...
- **完整国际化支持**：
  - 新增 `en-US` 和 `zh-CN` 双语言文案体系，覆盖主界面、设置、侧边栏、图表列表、PDF 查看器、主题切换和更新通知
  - 根据浏览器语言自动选择初始语言，并将用户选择持久化到本地存储
  - 切换语言时同步更新页面 `lang`、标题和描述信息
- **设置页语言切换器**：
  - 新增与设置弹窗视觉风格一致的自定义语言下拉菜单
  - 支持当前语言高亮、选中状态图标和键盘可访问的 listbox 语义

### 变更

- **图表分类与图表类型显示本地化**：
  - 侧边栏分类名称、图表列表标题、跑道分组、图表数量和图表类型说明会随语言切换
  - 保留航图名称、跑道编号和页码等业务数据原文，避免影响图表识别
- **构建流程同步 Rust crate 版本**：
  - GitHub Actions 构建时会从 `src-tauri/Cargo.toml` 同步 `chart-viewer` 在 `src-tauri/Cargo.lock` 中的版本号
  - 仅更新本项目包版本，不执行依赖升级

### 修复

- **修复 macOS 触控板平移体验问题**：
  - PDF 查看器手动缩放模式下支持触控板横向、纵向和对角线平移
  - 非缩放滚轮事件会按实际 `deltaX` / `deltaY` 滚动，避免只能单方向移动
- **修复 PDF 右侧边界无法查看的问题**：
  - 手动缩放模式下显式计算可滚动舞台宽高，确保右侧边界和内边距都包含在滚动范围内
  - PDF 页面容器禁止在 flex 布局中收缩，避免浏览器低估右侧可滚动宽度
...
- **优化 PDF 查看器滚轮缩放交互**：
  - Windows 下支持直接使用鼠标滚轮缩放图表，更符合桌面端查看器使用习惯
  - macOS 等平台继续支持触控板捏合手势触发的 `Ctrl` / `Meta` + 滚轮缩放
  - 非像素级滚轮事件会按鼠标滚轮处理，避免传统滚轮在非 Windows 平台无法缩放

### 修复

- **修复 PDF 查看器拖拽平移触发范围过宽的问题**：
  - 手动缩放模式下仅允许鼠标左键拖拽平移，避免右键或中键按下时误触发拖动状态
- **增强 CSV 图表解析的容错能力**：
  - 解析图表、每机场图表和机场信息 CSV 时统一清理字段首尾空格
  - 兼容缺失或非字符串字段，避免 `trim()` 调用导致解析中断
  - 补充资料和普通图表的文件引用检查统一使用安全字段读取逻辑
